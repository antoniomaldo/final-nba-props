...
public class CoefficientUpdatingOffRebPropFunction extends BaseCoefficientUpdatingFunction {
    ...
    protected List<PlayerPredictions> getPlayerPredictions(List<PlayerGameData> playerData, Double priorForPlayer, double[] par) {
        if (playerData.size() <= 1) {
            ...
            for (int i = 0; i < playerData.size(); i++) {
                ...
                targetPredicted = TargetPredicted.forOffRebProp(playerCoef, priorForPlayer);
                ...
                int totalRebounds = playerGameData.getRebounds();
                if (totalRebounds > 0) {
                    real = (double) playerGameData.getOffRebounds() / totalRebounds;
                    ...
                    playerCoef += Math.exp(weight2 * Math.max(0, 5 - numbGames)) * Math.exp(weight1) * Math.log(totalRebounds) * resid;
                } else{
