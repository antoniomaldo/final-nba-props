...
public class CoefficientUpdatingFoulsFunction extends BaseCoefficientUpdatingFunction {
    ...
    protected List<PlayerPredictions> getPlayerPredictions(List<PlayerGameData> playerData, Double priorForPlayer, double[] par) {
        if (playerData.size() <= 1) {
            ...
            for (int i = 0; i < playerData.size(); i++) {
                ...
                targetPerMinPredicted = TargetPredicted.forFouls(playerCoef, priorForPlayer);


                real = (double) playerGameData.getPersonalFouls() / playerGameData.getMinPlayed();
