...
public class CoefficientUpdatingAssistsFunction extends BaseCoefficientUpdatingFunction {
    ...
    protected List<PlayerPredictions> getPlayerPredictions(List<PlayerGameData> playerData, Double priorForPlayer, double[] par) {
        if (playerData.size() <= 1) {
            ...
            for (int i = 0; i < playerData.size(); i++) {
                ...
                targetPerMinPredicted = TargetPredicted.forAssists(playerCoef, priorForPlayer);

                real = (double) playerGameData.getAssists() / playerGameData.getMinPlayed();
