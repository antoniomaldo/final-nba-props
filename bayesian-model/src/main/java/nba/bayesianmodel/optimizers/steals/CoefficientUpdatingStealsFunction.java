...
public class CoefficientUpdatingStealsFunction extends BaseCoefficientUpdatingFunction {
    ...
    protected List<PlayerPredictions> getPlayerPredictions(List<PlayerGameData> playerData, Double priorForPlayer, double[] par) {
        if (playerData.size() <= 1) {
            ...
            for (int i = 0; i < playerData.size(); i++) {
                ...
                targetPerMinPredicted = TargetPredicted.forSteals(playerCoef, priorForPlayer);


                real = (double) playerGameData.getSteals() / playerGameData.getMinPlayed();
                ...
                playerCoef +=  Math.exp(weight1) * Math.log(playerGameData.getMinPlayed()) * resid;
                numbGames++;
            }
            return predictions;
        }
