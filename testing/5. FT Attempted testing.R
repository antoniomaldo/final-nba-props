...
javaPreds <- read.csv("C:\\czrs-ds-models\\nba-player-props\\testing\\ftsAttemptedPerMin.csv")
javaPreds <- subset(javaPreds, javaPreds$seasonYear > 2017 & javaPreds$target >= 0)
javaPreds <- merge(javaPreds, allPlayers[c("GameId", "PlayerId", "FT_Attempted", "Team", "HomeTeam", "AwayTeam",  "matchSpread", "totalPoints")])
...
javaPreds$ftExp <- javaPreds$targetPredicted * javaPreds$minPlayed
javaPreds$ftResid <- javaPreds$FT_Attempted - javaPreds$ftExp
...
binnedplot(javaPreds$ftExp, javaPreds$ftResid)
binnedplot(javaPreds$ownExp, javaPreds$ftResid)
