...
javaPreds <- read.csv("C:\\czrs-ds-models\\nba-player-props\\testing\\ofRebProp.csv")
javaPreds <- subset(javaPreds, javaPreds$seasonYear > 2017 & javaPreds$target >= 0)
javaPreds <- merge(javaPreds, allPlayers[c("GameId", "PlayerId", "Total_Rebounds", "Offensive_Rebound", "Team", "HomeTeam", "AwayTeam",  "matchSpread", "totalPoints")])
...
binnedplot(javaPreds$minPlayed, javaPreds$targetResid)

binnedplot(javaPreds$oppFgMissed, javaPreds$targetResid)
